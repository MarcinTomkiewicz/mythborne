...
import { PvpTargetingAdmin } from './pvp-targeting-admin';

describe('PvpTargetingAdmin', () => {
  let service: PvpTargetingAdmin;
  ...
  beforeEach(() => {
    ...
    metadata.getNamespaceEntries.and.callFake((namespace) => of([
      metadataEntry(
        namespace as typeof PVP_TARGETING_SECTION_METADATA_NAMESPACE
          | typeof PVP_CONFIGURATOR_SECTION_METADATA_NAMESPACE,
        `${namespace}-target_protection`,
        'Target protection',
      ),
    ]));
    ...
    service = TestBed.inject(PvpTargetingAdmin);
  });

  it('loads formula admin data and PvP targeting metadata through existing read services', async () => {
...
function metadataEntry(
  namespace: typeof PVP_TARGETING_SECTION_METADATA_NAMESPACE
    | typeof PVP_CONFIGURATOR_SECTION_METADATA_NAMESPACE,
  key: string,
  label: string,
