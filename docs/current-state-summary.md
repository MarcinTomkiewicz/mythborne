...
| Exploration / trials / encounters | 35% | DB/RPC foundation and guarded frontend models/mappers exist. `/game/exploration` now has player-facing status/start, DB-backed graph/direction display, movement start, DB-time-gated step resolve, resolved outcome presentation and active challenge completion/auto-resolve UI. Reward display and full runtime smoke are still pending real DB data. |
