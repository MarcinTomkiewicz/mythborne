...
| Exploration / trials / encounters | 20% | DB/RPC foundation and guarded frontend models/mappers exist. `/game/exploration` now has player-facing status/start plus DB-backed graph/direction display and movement start, but step resolution, challenges and rewards are still pending. |
