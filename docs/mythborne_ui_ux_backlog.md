...
Updated: 2026-05-29 — UI-PVP-7 spy flow accepted
...
**Status:** Accepted/completed on 2026-05-29. PvP spy flow now uses canonical backend state and report handoff: after `Szpieguj`, `/game/vicinity` refreshes `get_active_pvp_action_offer(p_hero_id)` and renders a compact spy progress panel below the PvP header, using backend target/address fields, `phase_ends_at ?? arrives_at`, `remaining_seconds` only as fallback and the shared `GameBar` progress visual instead of `PendingTimerOracle`. When the spy timer elapses, the frontend calls `settle_due_pvp_spy_action` and `create_pvp_spy_game_report` once per action, then redirects to `/game/reports/:reportId` rather than rendering a spy report inline in Vicinity. Private/public report models and renderers support `spy_section_json` for `pvp_spy` reports without requiring combat/trial outcome sections, and noisy combat-missing sections are avoided. Active spy and spy report detail use `public/images/backgrounds/spy-background.png`; if `get_active_pvp_action_offer` returns `phase === 'returning'` for spy, Vicinity stops with a DB/RPC blocker message instead of rendering return. No old `pvp-spy-result-*` pages/routes, direct PvP/combat/report table reads, `metadata_json` / resource-outcome parsing, attack flow, combat flow, reward flow or Oracle attack behavior were added/changed. Verification passed with `npx tsc --noEmit`, `npm run build` with known budget/CommonJS warnings and `git diff --check`; final manual smoke is user-side.
