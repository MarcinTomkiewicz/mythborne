...
**Status:** Accepted/completed on 2026-05-22 as the Exploration runtime challenge handoff shell and cleanup pass. Active challenges now render through one main `ExplorationChallengePanel` instead of legacy generic Challenge/Reward/Next-action cards, while sandbox/admin tools and selection diagnostics stay gated behind sandbox access. Runtime success feedback uses toasts and blocking errors remain inline. The resolved-step handoff was moved into a report-like production component under `src/app/game/components`, with player-facing outcome/flavor/reward/effect hierarchy and the accepted direction board kept as the continuation surface when movement is available. Combat challenges keep the existing live-combat flow only: ensure/start the session, show/start Walking Dead when the returned live state is `awaitingPlayerAction` with a timing manifest, submit through `submitPlayerAction(...)`, and refresh exploration/challenge state after completion. The Exploration UI does not use `advance_combat_live_to_next_player_action`, does not add a `Kontynuuj walkę` continuation flow, and treats a non-awaiting/non-completed combat session as a DB/runtime data issue for Migrator rather than pushing it forward from Angular. Verification passed with `npx tsc --noEmit`, `npm run build` with known bundle/CommonJS warnings and static greps; focused Exploration specs remain blocked before execution by unrelated existing compile errors in `src/app/core/utils/armory-inventory-filter.spec.ts`. Manual smoke remains user-side for representative active Trial, combat awaiting-action, combat non-awaiting/completed and resolved-step reward/effect states.
