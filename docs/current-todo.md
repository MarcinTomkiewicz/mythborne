...
- S13-S16 follow-up: `armory-page.html` and `armory-page.ts` are still heavy even after extracting loadout preset management and preview. The next larger Armory touch should extract bulk selection, equipment journal and shelf item card concerns into smaller components or dedicated state. `HeroEquipment` is intentionally hero-scoped after the S14 rename, but it is already broad; S15/S16 keep preset UI workflow in `HeroLoadoutPresetsState` instead of expanding `ArmoryPage`. S17 remains separate for apply preset.
