...
- UI-ENTRY-22-2A is accepted as the current public landing baseline: `/` keeps the shared route-background path from UI-ENTRY-22-2, uses the centered Mythsworn banner with shared `mg-brand-banner` treatment, top-aligned desktop hero copy, main CTAs, and right info cards. Homepage content is centralized in `src/app/core/config/home-page-content.config.ts` with interfaces in `src/app/core/interfaces/home-page-content.interface.ts` and stable template keys. `Stwórz bohatera` continues through `/auth/server-entry`; no DB/RPC, guards, generated types, start-flow, route-background or mobile/responsive behavior changed.
