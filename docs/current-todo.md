...
- Next backlog task: P11 - Reports Center prototype UI smoke and blocker report from `docs/codex-mythborne-backlog.md`, after the user commit and explicit start instruction.
- P2-P10 follow-ups: full manual smoke for list/detail/public-route/combat-rendering/item-reference rendering/unread/remove, low-level combat report creation, reward-drop attachment and contextual trial/encounter/PvP/siege readiness needs representative report producer/combat result/item/contextual data.
