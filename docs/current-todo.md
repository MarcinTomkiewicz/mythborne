...
- UI-ENTRY-22-2 is accepted with visual follow-up: `/` now renders the public Mythsworn landing baseline, `/auth/server-entry` and `/auth/create-character` remain under `AccountEntryLayout`, and landing/entry backgrounds use the shared AppShell route-background path with config in `core/config` and override service in `core/services/ui`. `Stwórz bohatera` continues through `/auth/server-entry`; no DB/RPC, guards, generated types, start-flow or mobile/responsive work changed. Next: UI-ENTRY-22-2A desktop visual polish only.
