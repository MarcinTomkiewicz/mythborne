...
- UI-EXPLORATION-11 is accepted: active challenge runtime now uses a single challenge handoff panel, sandbox tools and diagnostics are sandbox-gated, success feedback is toast-based, resolved-step handoff pieces live under `src/app/game/components`, and combat challenge UI preserves the existing ensure/start -> Walking Dead when DB awaits player input -> `submitPlayerAction(...)` -> refresh-after-completion flow. Exploration UI must not use `advance_combat_live_to_next_player_action` or add a `Kontynuuj walkę` frontend continuation; non-awaiting/non-completed live combat sessions are a DB/runtime data issue for Migrator. Pending smoke: representative active Trial, combat awaiting-action, combat non-awaiting/completed, resolved-step reward/effect and direction continuation states.
