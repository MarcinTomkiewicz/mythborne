...
- UI-EXPLORATION-12 Time Oracle follow-up is accepted: pending active movement now renders a shared presentational Time Oracle timer surface in the existing exploration runtime shell, with full-width outer card, centered circular oracle, coherent countdown/progress source, central pending/ready card, and a single ready-state `Sprawdź wynik` action. Direction/result/report cards remain hidden while the step is pending; result checking still uses the existing backend workflow readiness/action path. The direction-board continuation now has one shared board/footer block and a secondary `Zmień poziom trudności` action that returns to Difficulty preview only when no active step/challenge is running, without DB/RPC flow. Manual smoke remains user-side pending for representative pending/ready steps, Hard duration behavior, sandbox `Skróć czas` and the footer return action.
