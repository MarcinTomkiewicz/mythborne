...
- UI-COMBAT-1 and UI-COMBAT-2 are superseded by the accepted UI-COMBAT-3 integration: the prototype `/game/combat` shell and sandbox-only resolver/harness code have been removed as a product surface, while the accepted left combatant / center action / right opponent layout now lives on the real Exploration/Trial live combat surface.
- UI-COMBAT-3 is accepted: Exploration/Trial combat now renders the Combat Minigame Prototype structure with participant HP panels, central Walking Dead timing, player-facing submit copy, completed/no-manifest states and separate auto-resolve. Walking Dead keeps the existing live combat timing flow and submits only timing input; no resolver/RPC/generated-type/damage/outcome/log/report flow was changed. The meter SCSS/HTML cleanup is accepted with shared timing mixins, radius/opacity utilities, reduced-motion handling and no local timing engine. Participant stat cards/read-model rows are explicitly a separate follow-up.
