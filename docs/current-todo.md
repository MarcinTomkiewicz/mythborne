...
- Next backlog task: P10 - Future PvP and siege report placeholders from `docs/codex-mythborne-backlog.md`, after the user commit and explicit start instruction.
- P2-P9 follow-ups: full manual smoke for list/detail/public-route/combat-rendering/item-reference rendering/unread/remove, low-level combat report creation, reward-drop attachment and contextual trial/encounter readiness needs representative report producer/combat result/item/trial/encounter data.
