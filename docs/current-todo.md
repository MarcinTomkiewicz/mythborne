...
- Epic X Onboarding / Start Flow is accepted through X1 as of 2026-05-11. X1 added the typed start-flow integration layer over canonical `get_start_flow_server_availability`, `get_start_flow_origin_options` and `create_hero_start_flow`, replaced the legacy Angular direct-write hero bootstrap with the atomic DB/RPC creation workflow, reloads `ActiveHero` through selected server -> active hero after creation, blocks unsupported DB `route_next_action` values, and shows explicit origin option loading/error/empty states. DB/RPC owns starting Character Points, CP ledger, random district-A estate, starting stats/resources and route handoff. Origin bonus display uses DB `bonus_summary_text`; origin artwork remains the existing static carousel convention derived from `origin_key`, not DB-backed artwork. Manual/browser smoke remains pending until login/logout and sandbox multi-hero creation paths are clear.
