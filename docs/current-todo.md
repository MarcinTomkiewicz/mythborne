...
- UI-COMBAT-8 is accepted for the visual combat/minigame integration scope: Exploration routes combat challenges through `MinigameHost`, `CombatHost` stays thin, `CombatHostState` owns runtime/facade flow through `CombatSessions`, and active combat keeps the existing `CombatStage` / `CombatSurface` / `CombatLogPanel` / `CombatLogRow` path with the old `CombatDisplayLogRow` actor/body/result contract. Backend structured live event fields are mapped into that existing row shape; no new active `combat-report-log.mapper.ts` path, renderer contract, local parser, outcome inference, damage/healing construction or report producer flow is accepted in this slice. Follow-up: manual combat completion currently emits `reportId: null`, so Exploration can still show `Raport minigierki nie jest jeszcze dostępny` until the report handoff contract is fixed.
