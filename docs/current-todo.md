...
- UI-ENTRY-22-2A, UI-ENTRY-22-2B, UI-ENTRY-22-3, UI-ENTRY-22-4 and UI-ENTRY-22-5 are accepted as the current public landing/account-entry/hero-creation entry baseline. `/` keeps the shared route-background path, centered Mythsworn banner, top-aligned desktop hero copy, main CTAs and right info cards; homepage content is centralized in `src/app/core/config/home-page-content.config.ts` with interfaces in `src/app/core/interfaces/home-page-content.interface.ts`. The reusable `AppFooter` lives under `src/app/layout/components/app-footer`, uses config from `src/app/core/config/app-footer.config.ts`, renders only on non-game/non-admin shell routes, and links only to existing routes/aliases (`/`, `/auth/login`, `/register`, `/auth/server-entry`). `/auth/server-entry` and `/auth/create-character` share the `ENTRY_BACKGROUND_IMAGE` route-background mapping, stay under `AccountEntryLayout`, use existing `mg-card` / `mg-data-row` / utility surfaces, and no longer have `_account-entry.scss` or `mg-account-entry-*` selectors. Account entry and game shell sidebars now share `ShellSidebarContent`, with project-owned sidebar contracts in `src/app/core/interfaces/layout/sidebar.interface.ts`, game nav config in `src/app/core/config/menu-config.ts`, and account-entry nav/context config in `src/app/core/config/account-entry-sidebar.config.ts`. Hero creation keeps the canonical create-hero workflow while create-character config/display constants live in `src/app/core/config/create-character-server-options.config.ts`, shared summary rows use `AccountEntrySummaryRow`, and server/origin blocking loads use shared `LoadingOverlay`; manual smoke remains user-side. `Stwórz bohatera` continues through `/auth/server-entry`; no DB/RPC, guards, generated types, start-flow, route-background, route behavior, dashboard counters or mobile/responsive behavior changed.
