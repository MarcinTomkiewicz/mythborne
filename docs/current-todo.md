...
- UI-ITEMS-MOVE-1 is accepted/completed: Armory `Move selected` lives in the selected-items toolbar beside `Equip selected` and `Sell selected`, uses the canonical single-item `ArmoryShelfState.moveItemToShelf(...)` / `PlayerArmory.moveItemToShelf(...)` / `move_hero_armory_item_to_shelf` path, and is intentionally enabled only for exactly one eligible selected stored item because no canonical bulk move path exists. Destination options come from current shelf data and omit the current stand, the stale disabled state is fixed via `toSignal(moveTargetControl.valueChanges)`, selected item cards use global `.mg-card--selected`, and no per-card move selector, local SCSS, direct DB writes or generated-type edits remain. UI-ITEMS-MOVE-2 is also accepted/completed: eligible stored item cards can be dragged by grabbing the card surface and dropped onto another rendered stand/shelf, including empty stands, through Angular CDK drag/drop while reusing the same canonical move path; same-stand drops no-op, ineligible/guild/locked/non-active items are not draggable, and the toolbar `Move selected` fallback remains.
