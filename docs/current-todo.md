...
- S13 follow-up: `armory-page.html` and `armory-page.ts` are now heavy. The next larger Armory touch should extract bulk selection, equipment journal and shelf item card concerns into smaller components or dedicated state. `PlayerEquipment` remains naming debt; consider `HeroEquipment` / `HeroEquipmentService` during the next equipment cleanup.
