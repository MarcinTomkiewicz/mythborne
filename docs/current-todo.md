...
- UI-COMBAT-7 is accepted: completed combat reports keep rewards below the shared combat timeline, use the exploration reward handoff/display path, and keep report actions separate for internal full report and public copy/share. Public copy is always represented but copies only a backend-provided public path/token; missing public share data is shown as a safe disabled warning state, and the misleading same-route return action was removed. Backend/read-model follow-up: expose public report path/token for shareable completed combat reports and direct full report id/path when the action should open a specific report instead of report center.
