...
- UI-PVP-7 is accepted/completed: spy now stays in Vicinity only while travelling, using backend `get_active_pvp_action_offer(p_hero_id)` for a compact `GameBar` progress panel with target/address and arrival timing, then settles through `settle_due_pvp_spy_action`, creates a report through `create_pvp_spy_game_report`, and redirects to `/game/reports/:reportId`. `pvp_spy` reports render from safe `spy_section_json` without combat/trial outcome requirements, active spy and spy reports use the spy route background, and returning spy phases are treated as DB/RPC blockers. No old spy result routes/pages, direct table reads or raw metadata parsing were restored. Manual smoke remains user-side.
