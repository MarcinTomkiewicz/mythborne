...
- UI-ITEMS-11 is accepted/completed: visible Inventory item cards show lifecycle/guild badges only for non-default states, keeping `Active` and `Owned private item` hidden so default cards stay compact. Grid, card layout, popover, actions, TS services, DB/RPC and generated types remain frozen for this accepted scope. Full manual inventory smoke remains user-side/pending.
