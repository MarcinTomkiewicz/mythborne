...
- UI-PVP-1 is accepted pending final user-side smoke: `/game/vicinity` is now the PvP Vicinity target-selection screen with 20-address paginated vicinity rows, generated empty slots, visible target overlay RPC data, selected target panel, `Szpieguj` / `Atak` / disabled `Oblężenie`, protected danger/shield state, self success state, `Moja okolica`, relocation confirmation for empty addresses, daily attacks from RPC and own-estate attack/siege protection from `get_hero_estate_runtime_state`. PvP combat/result/report screens remain separate follow-ups.
