...
- Next backlog task: O10 - Player estate overview and building dashboard UI from `docs/codex-mythborne-backlog.md`, after the user commit and explicit start instruction.
...
- O9 follow-up: resource production after destructive relocation remains a non-blocking DB/runtime/topbar consistency issue. Check `hero_resources.per_hour` after relocation and mansion settlement; if DB remains stale, return to backend/migrator, and if DB is correct, fix topbar reload/cache. Do not add Angular-side production recalculation.
