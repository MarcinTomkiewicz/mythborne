...
- UI-COMBAT-1 is accepted: `/game/combat` now has the initial source-neutral combat shell, separate from PvP target selection and Exploration direction flow, with source/turn/result context, a central Walking Dead action surface, compact combatant cards, shared dashboard-extracted stat cards, and hero base/combat display rows sourced from the existing dashboard runtime display model. The sandbox caller remains the only active test surface; no DB/RPC/generated-type flow, Angular-side attack-count calculation, local stat label map or `advance_combat_live_to_next_player_action(...)` player flow was added. Manual smoke remains user-side pending for representative combat shell states.
- UI-COMBAT-2 is accepted: `/game/combat` uses the prototype combatant layout with left combatant, center action/Walking Dead card and right combatant. Combatant panels keep `GameBar` HP bars plus shared `app-stat-card` stat sections, show dictionary-backed side/kind labels, support active/final HP labels and keep attack-source log labeling deferred to later combat log/report work. The accepted scope does not include generated types, DB/RPC flow, production combat runtime changes or sandbox resolver logic. Manual smoke remains user-side pending for representative combatant panel and HP states.
