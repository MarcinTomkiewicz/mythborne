...
- Epic W Exploration Core is accepted through W2 as of 2026-05-10. W0 confirmed generated DB types expose the Exploration Core readiness, selection diagnostic, reward diagnostic, timer duration and sandbox helper contracts without frontend/generated-type edits. W1 added typed Trial/Encounter readiness, readiness reason metadata and step selection diagnostic domain models, generated RPC aliases and focused mappers/specs over DB/RPC rows. Selected Trial/Encounter diagnostics now require complete id+key identity; partial identities map to `null`. W2 aligned step resolution with canonical Trial/Encounter/Nothing outcomes, preserves raw DB outcome as diagnostics, maps DB-owned selected definition / selection diagnostic metadata, and lets `/game/exploration` distinguish Combat/Resource/Buff/Debuff Encounters only when DB metadata provides `encounterKind`. Manual smoke for W1 is N/A; W2 user-side manual smoke remains pending for Trial, Combat Encounter, Resource Encounter, Buff/Debuff Encounter and Nothing outcomes.
